...
void draw_hp_bar(int hp, int max_hp, int pokemon_id)
{
    ...
    // selecciona la posici�n de la barra
    if (pokemon_id == 0) {
        base_y = 17;
        base_x =  10;
    } else {
...
void display_string_in_box(const char *string) {
    ...
    for (int n = 0; n < len; n++)
    {
        ...
        // si encuentra un espacio o llega al final de la l�nea, imprime la palabra
        ...
        // si llega al final de la l�nea, reinicia las variables y baja una l�nea
        if (large == line_large)
        {
            x = 9;
            y = y + 10;
